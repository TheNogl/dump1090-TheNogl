...
//			var e = document.getElementsByName("plane_row_template2");
//			e.forEach(function(element) { element.addEventListener('click', selectPlaneByHex.bind(undefined,hex,false)); });
//			e.addEventListener('click', selectPlaneByHex.bind(undefined, hex, false));
//			e.addEventListener('dblclick', selectPlaneByHex.bind(undefined, hex, true));
...
function load_history_items()
{
	...
	for (var i = 0; i < PositionHistorySize; i++)
	{
		...
		.done(function(data)
		{
			...
			// don't care for order, will sort later
			loaded++;
			console.log("Loaded " + loaded + " history chunks");
			$("#loader_progress").attr('value', loaded);
			document.getElementById("progress-bar").style.width = loaded*100/PositionHistorySize + "%";
...
function initialize_map() {
	...
	if(mobile)
	{
		var e = document.getElementById("draggable-altitude-legend");
		e.style.display = "none";
		e = document.getElementById("draggable-range-legend");
		e.style.display = "none";
	}
}
...
function InitHeatmap()
{
	...
	if (HeatmapFileSelector)
	{
		...
		newEl.innerHTML = '<input type="file" id="csv-file" name="files" style="display: block;">';
	...
	{
		var newEl = document.createElement('p');
		newEl.innerHTML = '<b id="csv-file">Heatmap</b>';
		e.parentNode.replaceChild(newEl, e);
		//e.style.display = 'none';
	}
...
function loadKMLmap()
{
	if (UserMapShow)
	{
		UserKML = new google.maps.KmlLayer({ url: UserMap, map: GoogleMap, preserveViewport: true });
	}
	else
	{
		UserKML.setMap(null);
...
function loadKMLmap2()
{
	if (UserMapShow2)
	{
		UserKML2 = new google.maps.KmlLayer({
			url: UserMap2,
			map: GoogleMap,
			preserveViewport: true
		});
...
// This turns on or off the KML range map
function toggleRange2()
{
	UserMapShow2 = !UserMapShow2;
        loadKMLmap2();
}
	...
	if (MessageCountHistory.length > 1) {
		...
		if (message_time_delta > 0) {
			...
			if (date.getSeconds() % 10 == 0 || MessageRateChart.datasets[0].points.length < 3) {
				if (MessageRateChart.datasets[0].points.length > 600)
					MessageRateChart.removeData();
				if (PlanesRateChart.datasets[0].points.length > 600)
...
function refreshTableInfo() {
	...
	for (var i = 0; i < PlanesOrdered.length; ++i) {
		...
		if (!tableplane.visible) {
			...
			var hex = "'" + tableplane.icao + "'";
			if(follow_but) {
				tableplane.tr.cells[10].innerHTML = '<a align="right" class="btn btn-default btn-xs" onclick="follow(' + hex + ')">Stop Follow</a>'
			} else {
				tableplane.tr.cells[10].innerHTML = '<a align="right" class="btn btn-default btn-xs" onclick="follow(' + hex + ')">Follow</a>'
			}
//			tableplane.tr.cells[10].innerHTML = '<a class="btn btn-default btn-xs" onclick="selectPlaneByHex(' + tableplane.icao + ',true));">Follow</a>'
...
function follow(hex) {
	...
	//FollowSelected = true;
	selectPlaneByHex(hex,follow_but);
}
